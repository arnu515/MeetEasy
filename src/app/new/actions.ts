...
export async function create(_: unknown, fd: FormData) {
  ...
  const op = v.safeParse(schema, obj)
	...
	console.log(`${op.output.date}T${op.output.from}${op.output.offset}`, `${op.output.date}T${op.output.to}${op.output.offset}`)
	...
	if (fromDate.getDay() !== toDate.getDay() || fromDate.getMonth() !== toDate.getMonth() || fromDate.getFullYear() !== toDate.getFullYear()) return {
	  success: false,
	...
	if (fromDate <= now || toDate <= now) return {
	  success: false,
    error: "Invalid meeting date and time",
	...
	try {
	  const meeting = firstOrNull(await db.insert(meetings).values({
	    id: ulid(),
	  ...
	  console.error(e)
	  return { success: false, error: 'Database error', error_description: (e as Error).message || "Unknown error" }
	}
}
